...
def get_kv_cache_torch_dtype(
    ...
    if isinstance(cache_dtype, str):
        if cache_dtype in ("auto", "reflex_int4"):
            torch_dtype = _resolve_model_dtype(model_dtype)
...
def _resolve_model_dtype(model_dtype: str | torch.dtype | None) -> torch.dtype:
...
def kv_cache_dtype_str_to_dtype(
    ...
    if kv_cache_dtype in ("auto", "reflex_int4"):
