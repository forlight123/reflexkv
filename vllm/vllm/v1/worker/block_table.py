...
from vllm.v1.core.precision_kv.types import ReflexDemotion, ReflexRecovery
...
class MultiGroupBlockTable:
    ...
    def apply_reflex_int4_demotions(
        self,
        *,
        row_by_req_id: dict[str, int],
        demotions: list[ReflexDemotion],
    ) -> None:
        for demotion in demotions:
            row_idx = row_by_req_id.get(demotion.request_id)
            if row_idx is None:
                continue
            self.update_block_id(
                ...
                block_id=demotion.encoded_block_table_id,
            )

    def apply_reflex_int4_recoveries(
        self,
        *,
        row_by_req_id: dict[str, int],
        ...
        for recovery in recoveries:
            row_idx = row_by_req_id.get(recovery.request_id)
            if row_idx is None:
                continue
            self.update_block_id(
                kv_cache_group_id=recovery.kv_cache_group_id,
                row_idx=row_idx,
                page_idx=recovery.page_idx,
                block_id=recovery.encoded_block_table_id,
            )
