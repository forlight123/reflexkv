...
from vllm.v1.core.reflex_prefill_metadata import (
    ...
    reflex_prefill_metadata_enabled,
)
...
def _apply_reflex_int4_demotions_to_request_states(
    requests: dict[str, CachedRequestState],
    demotions: list[ReflexDemotion],
) -> None:
    for demotion in demotions:
        req_state = requests.get(demotion.request_id)
        if req_state is None:
            continue
        block_ids_by_group = getattr(req_state, "block_ids", None)
        if block_ids_by_group is None:
            continue
        if demotion.kv_cache_group_id >= len(block_ids_by_group):
            continue
        block_ids = block_ids_by_group[demotion.kv_cache_group_id]
        if demotion.page_idx >= len(block_ids):
            continue
        block_ids[demotion.page_idx] = demotion.encoded_block_table_id


def _apply_reflex_int4_recoveries_to_request_states(
    requests: dict[str, CachedRequestState],
    recoveries: list[ReflexRecovery],
) -> None:
    for recovery in recoveries:
        req_state = requests.get(recovery.request_id)
        if req_state is None:
            continue
        block_ids_by_group = getattr(req_state, "block_ids", None)
        if block_ids_by_group is None:
            continue
        if recovery.kv_cache_group_id >= len(block_ids_by_group):
            continue
        block_ids = block_ids_by_group[recovery.kv_cache_group_id]
        if recovery.page_idx >= len(block_ids):
            continue
        block_ids[recovery.page_idx] = recovery.encoded_block_table_id
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.kv_cache_by_layer_name: dict[str, torch.Tensor] = {}
        self.reflex_int4_kv_caches: dict[str, torch.Tensor] = {}
        self.reflex_int4_group_layer_names: dict[int, list[str]] = {}
        self.reflex_bf16_shadow_store: dict[
    ...
    def _get_reflex_int4_block_table_stats(self, num_reqs: int):
        block_table = self.input_batch.block_table[0]
        return summarize_reflex_block_table(
            block_table.get_numpy_array(),
            block_table.num_blocks_per_row,
            num_rows=num_reqs,
        )
    ...
    def _has_reflex_int4_blocks(self, kv_cache_gid: int, num_reqs: int) -> bool:
        ...
        for row in range(num_reqs):
            active_blocks = int(num_blocks_per_row[row])
    ...
    def _reflex_prefill_metadata_context(
        ...
        *,
        ...
        if not reflex_prefill_metadata_enabled() or num_reqs <= 0:
            yield
            return
        ...
        for req_idx, req_id in enumerate(req_ids):
            ...
            if not kv_params.get("do_remote_decode"):
                continue
            ...
            query_start = int(self.query_start_loc.np[req_idx])
            infos.append(
                ReflexPrefillRequestInfo(
                    request_id=req_id,
                    query_start=query_start,
                    query_end=query_start + (prompt_end - computed_tokens),
                    prompt_start=computed_tokens,
    ...
    def _execute_reflex_int4_demotions(self, demotions: list[ReflexDemotion]) -> None:
        ...
        layer_copies = 0
        kernel_launches = 0
        demotions_by_group: dict[int, list[ReflexDemotion]] = defaultdict(list)
        for demotion in demotions:
            demotions_by_group[demotion.kv_cache_group_id].append(demotion)
        for kv_cache_group_id, group_demotions in demotions_by_group.items():
            layer_names = self.reflex_int4_group_layer_names.get(kv_cache_group_id, [])
            if not layer_names:
                continue
            first_cache = self.kv_cache_by_layer_name[layer_names[0]]
            src_block_ids = torch.tensor(
                [demotion.bf16_block_id for demotion in group_demotions],
                dtype=torch.int64,
                device=first_cache.device,
            )
            dst_block_ids = torch.tensor(
                [demotion.int4_block_id for demotion in group_demotions],
                dtype=torch.int64,
                device=first_cache.device,
            )
            for layer_name in layer_names:
                bf16_cache = self.kv_cache_by_layer_name[layer_name]
                int4_cache = self.reflex_int4_kv_caches[layer_name]
                self._store_reflex_bf16_shadow_pages(
                    layer_name,
                    bf16_cache,
                    group_demotions,
                )
                ...
                layer_copies += len(group_demotions)
                kernel_launches += 1
        if trace_enabled:
            gpu_ms = -1.0
            ...
            logger.info(
                "ReFlexKV trace demote_exec pages=%d layer_copies=%d "
                "kernel_launches=%d cpu_ms=%.3f gpu_ms=%.3f.",
                len(demotions),
                layer_copies,
                kernel_launches,
    ...
    def _store_reflex_bf16_shadow_pages(
        self,
        layer_name: str,
        ...
        for demotion in demotions:
            if demotion.recovery_class != RecoveryClass.BF16_SHADOW:
                continue
            if demotion.bf16_block_id >= bf16_cache.shape[0]:
                raise RuntimeError(
                    "Cannot store ReFlexKV BF16 shadow because the source "
                    f"block is out of range: layer={layer_name}, "
                    f"block={demotion.bf16_block_id}, "
                    f"num_blocks={bf16_cache.shape[0]}."
                )
            key = (
                demotion.kv_cache_group_id,
                demotion.request_id,
                demotion.page_idx,
                layer_name,
            )
    ...
    def _execute_reflex_int4_recoveries(
        ...
        if not recoveries or self.cache_config.cache_dtype != "reflex_int4":
            return
        trace_enabled = self.reflex_int4_trace_enabled
        cpu_start = time.perf_counter()
        recoveries_by_group: dict[int, list[ReflexRecovery]] = defaultdict(list)
        for recovery in recoveries:
            recoveries_by_group[recovery.kv_cache_group_id].append(recovery)

        layer_copies = 0
        for kv_cache_group_id, group_recoveries in recoveries_by_group.items():
            layer_names = self.reflex_int4_group_layer_names.get(kv_cache_group_id, [])
            if not layer_names:
                continue
            for layer_name in layer_names:
                bf16_cache = self.kv_cache_by_layer_name[layer_name]
                for recovery in group_recoveries:
                    if recovery.recovery_class != RecoveryClass.BF16_SHADOW:
                        continue
                    if recovery.bf16_block_id >= bf16_cache.shape[0]:
                        raise RuntimeError(
                            "Cannot recover ReFlexKV BF16 shadow because the "
                            f"destination block is out of range: layer={layer_name}, "
                            f"block={recovery.bf16_block_id}, "
                            f"num_blocks={bf16_cache.shape[0]}."
                        )
                    key = (
                        recovery.kv_cache_group_id,
                        recovery.request_id,
                        recovery.page_idx,
                        layer_name,
                    )
                    shadow = self.reflex_bf16_shadow_store.pop(key, None)
                    if shadow is None:
                        raise RuntimeError(
                            "Cannot recover ReFlexKV INT4 page because its "
                            f"BF16 shadow is missing: request={recovery.request_id}, "
                            f"page={recovery.page_idx}, layer={layer_name}."
                        )
                    bf16_cache[recovery.bf16_block_id].copy_(
                        shadow.to(
                            device=bf16_cache.device,
                            dtype=bf16_cache.dtype,
                        ),
                        non_blocking=True,
                    )
                    layer_copies += 1
    ...
    def _discard_reflex_bf16_shadows_for_requests(
        self,
        request_ids: set[str],
    ) -> None:
        if not request_ids or not getattr(self, "reflex_bf16_shadow_store", None):
            return
        self.reflex_bf16_shadow_store = {
            key: value
            for key, value in self.reflex_bf16_shadow_store.items()
            if key[1] not in request_ids
        }
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        self._discard_reflex_bf16_shadows_for_requests(
            scheduler_output.finished_req_ids
        )
        ...
        if scheduler_output.reflex_int4_demotions:
            ...
            self.input_batch.block_table.apply_reflex_int4_demotions(
                row_by_req_id=self.input_batch.req_id_to_index,
                demotions=scheduler_output.reflex_int4_demotions,
            )

        if scheduler_output.reflex_int4_recoveries:
            self._execute_reflex_int4_recoveries(
            ...
            self.input_batch.block_table.apply_reflex_int4_recoveries(
                row_by_req_id=self.input_batch.req_id_to_index,
                recoveries=scheduler_output.reflex_int4_recoveries,
            )
    ...
    def _build_attention_metadata(
        ...
        for kv_cache_gid, kv_cache_group in enumerate(kv_cache_groups):
            ...
            if kv_cache_gid > 0:
                ...
                cm.has_reflex_int4_blocks = self._has_reflex_int4_blocks(
                    kv_cache_gid, num_reqs
                )
    ...
    def execute_model(
        ...
        trace_enabled = (
            ...
            and self.cache_config.cache_dtype == "reflex_int4"
        )
        if trace_enabled:
            trace_step = self._reflex_int4_trace_step
            self._reflex_int4_trace_step += 1
        else:
            trace_step = -1
        preprocess_cpu_start = time.perf_counter()
        with (
        ...
        preprocess_cpu_ms = (time.perf_counter() - preprocess_cpu_start) * 1000.0
        ...
        with (
            ...
            self._reflex_prefill_metadata_context(
                num_reqs=num_reqs,
                num_scheduled_tokens=scheduler_output.num_scheduled_tokens,
            ),
        ...
        if forward_start_event is not None and forward_end_event is not None:
            ...
            forward_gpu_ms = forward_start_event.elapsed_time(forward_end_event)
        forward_cpu_ms = (time.perf_counter() - forward_cpu_start) * 1000.0

        postprocess_cpu_start = time.perf_counter()
        postprocess_gpu_ms = -1.0
        postprocess_start_event = postprocess_end_event = None
        if trace_enabled and self.device.type == "cuda":
            postprocess_start_event = torch.cuda.Event(enable_timing=True)
            postprocess_end_event = torch.cuda.Event(enable_timing=True)
            postprocess_start_event.record()
        ...
        if postprocess_start_event is not None and postprocess_end_event is not None:
            postprocess_end_event.record()
            postprocess_end_event.synchronize()
            postprocess_gpu_ms = postprocess_start_event.elapsed_time(
                postprocess_end_event
            )
        postprocess_cpu_ms = (time.perf_counter() - postprocess_cpu_start) * 1000.0
        ...
        if trace_enabled:
            block_stats = self._get_reflex_int4_block_table_stats(num_reqs)
            trace_max_seq_len = int(
                self.optimistic_seq_lens_cpu.numpy()[:num_reqs].max().item()
            )
            budget = self.reflex_int4_budget
            bf16_capacity_blocks = int(
                budget.get("bf16_num_blocks", self.kv_cache_config.num_blocks)
            )
            int4_capacity_blocks = int(budget.get("int4_num_blocks", 0))
            bf16_budget_bytes = int(budget.get("bf16_budget_bytes", 0))
            int4_budget_bytes = int(budget.get("int4_budget_bytes", 0))
            int4_budget_fraction = float(budget.get("int4_budget_fraction", 0.0))
            int4_ratio = (
                block_stats.int4_blocks / block_stats.total_blocks
                if block_stats.total_blocks
                else 0.0
            )
            logger.info(
                "ReFlexKV trace step=%d phase=%s reqs=%d scheduled_tokens=%d "
                "max_query_len=%d max_seq_len=%d demotions=%d "
                "kv_blocks_total=%d kv_blocks_bf16=%d kv_blocks_int4=%d "
                "bf16_capacity_blocks=%d int4_capacity_blocks=%d "
                "bf16_budget_bytes=%d int4_budget_bytes=%d "
                "int4_budget_fraction=%.4f "
                "kv_int4_ratio=%.4f preprocess_cpu_ms=%.3f "
                "forward_cpu_ms=%.3f forward_gpu_ms=%.3f "
                "postprocess_cpu_ms=%.3f postprocess_gpu_ms=%.3f.",
                trace_step,
                "prefill_or_mixed" if max_num_scheduled_tokens > 1 else "decode",
                num_reqs,
                num_tokens_unpadded,
                max_num_scheduled_tokens,
                trace_max_seq_len,
                len(scheduler_output.reflex_int4_demotions or ()),
                block_stats.total_blocks,
                block_stats.bf16_blocks,
                block_stats.int4_blocks,
    ...
    def initialize_kv_cache_tensors(
        ...
        self.kv_cache_by_layer_name = kv_caches
        self._initialize_reflex_int4_kv_caches(kv_cache_config, kv_caches)
        return kv_caches

    def _initialize_reflex_int4_kv_caches(
        self,
        kv_cache_config: KVCacheConfig,
        kv_caches: dict[str, torch.Tensor],
    ) -> None:
        self.reflex_int4_kv_caches = {}
        self.reflex_int4_group_layer_names = {}
        self.reflex_int4_budget = dict(kv_cache_config.reflex_int4_budget or {})
        if not self._needs_reflex_int4_kv_caches():
            return

        forward_context = self.compilation_config.static_forward_context
        kv_cache_to_int4_cache: dict[int, torch.Tensor] = {}
        int4_num_blocks = kv_cache_config.reflex_int4_num_blocks
        for group_id, group in enumerate(kv_cache_config.kv_cache_groups):
            ...
            for layer_name in group.layer_names:
                ...
                if not isinstance(spec, AttentionSpec):
                    continue
                kv_cache = kv_caches[layer_name]
                int4_shape = self._get_reflex_int4_cache_shape(
                    kv_cache,
                    num_blocks=int4_num_blocks,
                )
    ...
    def _get_reflex_int4_cache_shape(
        ...
        if kv_cache.shape[1] == 2:
            ...
            source_num_blocks = kv_cache.shape[1]
            block_size = kv_cache.shape[2]
            num_kv_heads = kv_cache.shape[3]
            head_size = kv_cache.shape[4]
        else:
            raise ValueError(
                "ReFlexKV INT4 staging expects K/V as either dimension 0 "
                "or dimension 1."
            )
        if num_blocks is None:
            num_blocks = int(source_num_blocks)
        return (
            num_blocks,
            2,
            block_size,
            num_kv_heads,
            get_reflex_int4_codec().packed_head_size_bytes(head_size),
        )

    def _needs_reflex_int4_kv_caches(self) -> bool:
        return self.cache_config.cache_dtype == "reflex_int4"
    ...
    def _register_kv_caches_with_transfer_group(
        self,
        kv_transfer_group: Any,
        kv_caches: dict[str, torch.Tensor],
    ) -> None:
        ...
        if (
            self.cache_config.cache_dtype == "reflex_int4"
            and self.reflex_int4_kv_caches
        ):
            register_int4_caches = getattr(
                kv_transfer_group,
                "register_reflex_int4_kv_caches",
                None,
            )
            if callable(register_int4_caches):
                register_int4_caches(self.reflex_int4_kv_caches)
