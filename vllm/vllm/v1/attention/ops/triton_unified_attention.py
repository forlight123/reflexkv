...
def kernel_unified_attention_2d(
    ...
    reflex_int4_cache_ptr,  # [num_i4_blks, 2, blk_size, num_kv_heads, packed_head]
    ...
    stride_reflex_int4_cache_0: tl.int64,  # int
    stride_reflex_int4_cache_1: tl.int64,  # int
    stride_reflex_int4_cache_2: tl.int64,  # int
    stride_reflex_int4_cache_3: tl.int64,  # int
    stride_reflex_int4_cache_4: tl.constexpr,  # int
    query_start_len_ptr,  # [num_seqs+1]
    BLOCK_Q: tl.constexpr,  # int
    num_seqs: tl.int32,
    BLOCK_M: tl.constexpr,  # int
    USE_FP8: tl.constexpr,  # bool
    USE_REFLEX_INT4: tl.constexpr,  # bool
        ...
        if USE_REFLEX_INT4:
            ...
            if K_load.dtype.is_fp8():
                if Q.dtype.is_fp8():
                    ...
                    K_bf16 = (K_load.to(tl.float32) * tl.load(k_scale)).to(Q.dtype)
            else:
                K_bf16 = K_load
            ...
            scale_byte = tl.load(
                ...
                mask=tile_mask[None, :] & is_reflex_int4[None, :],
                other=127,
            ).to(tl.float32)
            K_int4 = signed * tl.exp2(scale_byte - 127.0)
            K = tl.where(is_reflex_int4[None, :], K_int4.to(Q.dtype), K_bf16)
        else:
            # K : (HEAD_SIZE, TILE_SIZE)
            K_load = tl.load(
                key_cache_ptr + k_offset,
        ...
        if USE_REFLEX_INT4:
            ...
            if V_load.dtype.is_fp8():
                if Q.dtype.is_fp8():
                    ...
                    V_bf16 = (V_load.to(tl.float32) * tl.load(v_scale)).to(Q.dtype)
            else:
                V_bf16 = V_load
            ...
            V_int4 = signed * tl.exp2(scale_byte - 127.0)
            V = tl.where(is_reflex_int4[:, None], V_int4.to(Q.dtype), V_bf16)
        else:
            # V : (TILE_SIZE, HEAD_SIZE)
            V_load = tl.load(
                value_cache_ptr + v_offset,
...
def unified_attention(
    ...
    use_reflex_int4 = reflex_int4_kv_cache is not None
    ...
    trace_attention = _trace_attention
    trace_int4_entries = 0
    if trace_attention and block_table.numel() > 0:
        trace_int4_entries = int(torch.count_nonzero(block_table < 0).item())
    ...
    if (
        ...
        trace_cpu_start = time.perf_counter() if trace_attention else 0.0
        trace_start_event = trace_end_event = None
        if trace_attention and q.is_cuda:
            trace_start_event = torch.cuda.Event(enable_timing=True)
            trace_end_event = torch.cuda.Event(enable_timing=True)
            trace_start_event.record()
        kernel_unified_attention_2d[
            ...
            stride_reflex_int4_cache_0=(
                reflex_int4_kv_cache.stride(0) if use_reflex_int4 else 0
            ),
            stride_reflex_int4_cache_1=(
                reflex_int4_kv_cache.stride(1) if use_reflex_int4 else 0
            ),
            stride_reflex_int4_cache_2=(
                reflex_int4_kv_cache.stride(2) if use_reflex_int4 else 0
            ),
            stride_reflex_int4_cache_3=(
                reflex_int4_kv_cache.stride(3) if use_reflex_int4 else 0
            ),
            stride_reflex_int4_cache_4=(
                reflex_int4_kv_cache.stride(4) if use_reflex_int4 else 0
            ),
            ...
            USE_REFLEX_INT4=use_reflex_int4,
        )
        if trace_attention:
            trace_gpu_ms = -1.0
            if trace_start_event is not None and trace_end_event is not None:
                trace_end_event.record()
                trace_end_event.synchronize()
                trace_gpu_ms = trace_start_event.elapsed_time(trace_end_event)
            logger.info(
                "ReFlexKV trace attention kernel=2d use_reflex_int4=%s "
                "q_tokens=%d num_seqs=%d max_q=%d max_k=%d kv_heads=%d "
                "total_q_blocks=%d tile_size=%d int4_entries=%d "
                "cpu_ms=%.3f gpu_ms=%.3f.",
                use_reflex_int4,
                q.shape[0],
                num_seqs,
                ...
                TILE_SIZE_PREFILL,
                trace_int4_entries,
                (time.perf_counter() - trace_cpu_start) * 1000.0,
                trace_gpu_ms,
            )
    else:
        trace_cpu_start = time.perf_counter() if trace_attention else 0.0
        ...
        if trace_attention:
            ...
            if trace_start_event is not None and trace_end_event is not None:
                ...
                trace_gpu_ms = trace_start_event.elapsed_time(trace_end_event)
            logger.info(
                "ReFlexKV trace attention kernel=3d use_reflex_int4=%s "
                "q_tokens=%d num_seqs=%d max_q=%d max_k=%d kv_heads=%d "
                "total_q_blocks=%d tile_size=%d int4_entries=%d "
                "cpu_ms=%.3f gpu_ms=%.3f.",
                use_reflex_int4,
