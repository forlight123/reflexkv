...
def _uses_reflex_int4(kv_cache_spec: KVCacheSpec) -> bool:
    if getattr(kv_cache_spec, "cache_dtype_str", None) == "reflex_int4":
        return True
    child_specs = getattr(kv_cache_spec, "kv_cache_specs", None)
    if child_specs is None:
        return False
    return any(
        getattr(child_spec, "cache_dtype_str", None) == "reflex_int4"
...
class KVCacheCoordinator(ABC):
    ...
    def __init__(
        ...
        for i, kv_cache_group in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            if _uses_reflex_int4(kv_cache_group.kv_cache_spec):
    ...
    def get_block_ids(self, request_id: str) -> tuple[list[int], ...]:
        return tuple(
            list(manager.get_reflex_block_ids(request_id))
            for manager in self.single_type_managers
        )

    def has_reflex_int4_blocks(self, request_id: str) -> bool:
        return any(
            manager.has_reflex_int4_blocks(request_id)
            for manager in self.single_type_managers
        )
    ...
    def reserve_reflex_int4_landing_blocks(
        ...
        for manager in self.single_type_managers:
            if getattr(manager, "reflex_int4_pool", None) is not None:
                return manager.reserve_reflex_int4_landing_blocks(
                    request_id,
                    count,
                )
        return []
    ...
    def record_reflex_int4_landing_pages(
        ...
        for manager in self.single_type_managers:
            if getattr(manager, "reflex_int4_pool", None) is not None:
                manager.record_reflex_int4_landing_pages(
                    request_id,
                    page_indices,
                )
                return
    ...
    def release_reflex_int4_landing_blocks(self, request_id: str) -> None:
        for manager in self.single_type_managers:
            manager.release_reflex_int4_landing_blocks(request_id)
    ...
    def commit_reflex_int4_landing_pages(
        ...
        for manager in self.single_type_managers:
            if getattr(manager, "reflex_int4_pool", None) is None:
                continue
            landing_ids = getattr(
                manager,
                "req_to_reflex_landing_int4_ids",
                {},
            ).get(request_id, ())
            if landing_ids:
                return manager.commit_reflex_int4_landing_pages(
                    request_id,
                    page_indices,
                    int4_block_ids,
                )
    ...
    def recover_reflex_int4_pages(
        ...
        for manager in self.single_type_managers:
            if getattr(manager, "reflex_int4_pool", None) is None:
                continue
            recovered = manager.recover_reflex_int4_pages(
                request_id,
                page_indices,
            )
            if recovered:
    ...
    def get_reflex_page_runtime_descriptors(
        ...
        **kwargs,
        ...
        for manager in self.single_type_managers:
            ...
            if callable(get_descriptors):
                descriptors.extend(get_descriptors(request_id, **kwargs))
        return descriptors
    ...
    def plan_reflex_int4_demotions(
        ...
        *,
        ...
        remaining = target_bf16_blocks
        released = 0
        for manager in self.single_type_managers:
            manager_released = manager.plan_reflex_int4_demotions(
                target_bf16_blocks=max(0, remaining),
            ...
            released += manager_released
            remaining -= manager_released
        return released

    def get_last_reflex_int4_candidate_capacity(self) -> int:
        return sum(
            manager.get_last_reflex_int4_candidate_capacity()
            for manager in self.single_type_managers
            if hasattr(manager, "get_last_reflex_int4_candidate_capacity")
        )
    ...
    def get_last_reflex_int4_candidate_breakdown(
        ...
        for manager in self.single_type_managers:
            if hasattr(manager, "get_last_reflex_int4_candidate_breakdown"):
                breakdown += manager.get_last_reflex_int4_candidate_breakdown()
        return breakdown
    ...
    def get_reflex_precision_state_counts(
        ...
        for manager in self.single_type_managers:
            ...
            if not callable(get_counts):
                continue
            manager_counts = get_counts(request_id)
            for key, value in manager_counts.items():
    ...
    def take_reflex_int4_demotions(self) -> list[ReflexDemotion]:
        ...
        for manager in self.single_type_managers:
            demotions.extend(manager.take_reflex_int4_demotions())
        return demotions

    def take_reflex_int4_recoveries(self) -> list[ReflexRecovery]:
        recoveries: list[ReflexRecovery] = []
        for manager in self.single_type_managers:
            if hasattr(manager, "take_reflex_int4_recoveries"):
                recoveries.extend(manager.take_reflex_int4_recoveries())
        return recoveries
