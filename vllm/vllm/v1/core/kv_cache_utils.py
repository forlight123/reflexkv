...
def get_reflex_int4_sidecar_page_size(kv_cache_spec: KVCacheSpec) -> int:
    """Return extra per-block bytes for ReFlexKV INT4 sidecar storage."""
    if isinstance(kv_cache_spec, UniformTypeKVCacheSpecs):
        return sum(
            get_reflex_int4_sidecar_page_size(spec)
    ...
    if (
        ...
        packed_head_size = AttentionSpec.int4_packed_head_size_bytes(
...
def get_reflex_int4_effective_page_size(kv_cache_spec: KVCacheSpec) -> int:
    """Return page bytes including ReFlexKV sidecar bytes when applicable."""
    return kv_cache_spec.page_size_bytes + get_reflex_int4_sidecar_page_size(
        kv_cache_spec
    )


DEFAULT_REFLEX_INT4_BUDGET_FRACTION = 0.10
REFLEX_INT4_BUDGET_FRACTION_ENV = "SEMANTIQ_REFLEX_INT4_BUDGET_FRACTION"
REFLEX_INT4_AUTO_BF16_FLOOR_FRACTION = 0.90
REFLEX_INT4_AUTO_BF16_CEILING_FRACTION = 0.95


def get_reflex_int4_budget_fraction(vllm_config: VllmConfig) -> float | None:
    """Return explicit INT4 budget fraction, or None for auto budgeting."""
    cache_config = vllm_config.cache_config
    config_value = getattr(cache_config, "reflex_int4_budget_fraction", None)
    env_value = os.getenv(REFLEX_INT4_BUDGET_FRACTION_ENV)
    ...
    try:
        ...
        raise ValueError(
            f"{REFLEX_INT4_BUDGET_FRACTION_ENV} must be a float in (0, 1) "
            f"or 'auto'; got {raw_value!r}."
        ) from exc
    if not 0.0 < fraction < 1.0:
        raise ValueError(
            f"ReFlexKV INT4 budget fraction must be in (0, 1); got {fraction}."
        )
    return fraction


def get_reflex_int4_auto_budget_fraction(
    vllm_config: VllmConfig,
    kv_cache_spec: KVCacheSpec,
    available_memory: int,
) -> float:
    """Pick a BF16-first INT4 sidecar budget for long-context serving.

    The initial implementation used a fixed 50/50 byte split. That makes the
    BF16 landing pool too small for P/D long-context admission before the
    mixed-precision landing path has a chance to help. The auto policy keeps a
    small INT4 sidecar by default while preserving enough BF16 pages for at
    least one full-length admission gate.
    """
    bf16_total_blocks = max(1, available_memory // kv_cache_spec.page_size_bytes)
    model_config = getattr(vllm_config, "model_config", None)
    max_model_len = int(getattr(model_config, "max_model_len", 0) or 0)
    required_fraction = 0.0
    if max_model_len > 0:
        required_blocks = cdiv(max_model_len, kv_cache_spec.block_size)
        required_fraction = required_blocks / bf16_total_blocks

    bf16_fraction = max(
        REFLEX_INT4_AUTO_BF16_FLOOR_FRACTION,
        required_fraction,
    )
    bf16_fraction = min(
        REFLEX_INT4_AUTO_BF16_CEILING_FRACTION,
        bf16_fraction,
    )
    int4_fraction = 1.0 - bf16_fraction
    if int4_fraction <= 0.0:
        int4_fraction = DEFAULT_REFLEX_INT4_BUDGET_FRACTION
    else:
        int4_fraction = min(int4_fraction, DEFAULT_REFLEX_INT4_BUDGET_FRACTION)
    return round(int4_fraction, 6)


def get_reflex_int4_budgeted_block_counts(
    vllm_config: VllmConfig,
    kv_cache_spec: KVCacheSpec,
    available_memory: int,
) -> tuple[int, int, dict[str, int | float] | None]:
    """Split the KV budget into BF16 and INT4 capacities for ReFlexKV.

    Returns `(bf16_blocks, int4_blocks, budget_info)`. `budget_info` is `None`
    when the spec is not a ReFlexKV INT4 cache.
    """
    int4_page_size = get_reflex_int4_sidecar_page_size(kv_cache_spec)
    if int4_page_size == 0:
        num_blocks = available_memory // kv_cache_spec.page_size_bytes
        return num_blocks, 0, None

    int4_fraction = get_reflex_int4_budget_fraction(vllm_config)
    if int4_fraction is None:
        int4_fraction = get_reflex_int4_auto_budget_fraction(
            vllm_config,
            kv_cache_spec,
            available_memory,
        )
    bf16_fraction = 1.0 - int4_fraction
    bf16_page_size = kv_cache_spec.page_size_bytes
    bf16_budget_bytes = int(available_memory * bf16_fraction)
    int4_budget_bytes = available_memory - bf16_budget_bytes
    bf16_blocks = bf16_budget_bytes // bf16_page_size
    int4_blocks = int4_budget_bytes // int4_page_size
    budget_info: dict[str, int | float] = {
        "available_memory_bytes": available_memory,
        "bf16_budget_fraction": bf16_fraction,
        "int4_budget_fraction": int4_fraction,
        "bf16_budget_bytes": bf16_budget_bytes,
        "int4_budget_bytes": int4_budget_bytes,
        "bf16_page_size_bytes": bf16_page_size,
        "int4_page_size_bytes": int4_page_size,
        "bf16_num_blocks": int(bf16_blocks),
        "int4_num_blocks": int(int4_blocks),
    }
    return int(bf16_blocks), int(int4_blocks), budget_info


def rebudget_reflex_int4_for_bf16_blocks(
    kv_cache_spec: KVCacheSpec,
    *,
    available_memory: int,
    bf16_blocks: int,
    ...
    """Use memory left by a BF16 block override for the INT4 sidecar pool."""
    int4_page_size = get_reflex_int4_sidecar_page_size(kv_cache_spec)
    if int4_page_size <= 0:
        return 0, budget_info

    bf16_page_size = kv_cache_spec.page_size_bytes
    bf16_budget_bytes = min(
        max(0, int(available_memory)),
        max(0, int(bf16_blocks)) * bf16_page_size,
    )
    int4_budget_bytes = max(0, int(available_memory) - bf16_budget_bytes)
    int4_blocks = int4_budget_bytes // int4_page_size

    updated_budget = dict(budget_info)
    available_memory_float = float(max(1, int(available_memory)))
    updated_budget.update(
        {
            "available_memory_bytes": int(available_memory),
            "bf16_budget_fraction": bf16_budget_bytes / available_memory_float,
            "int4_budget_fraction": int4_budget_bytes / available_memory_float,
            "bf16_budget_bytes": int(bf16_budget_bytes),
            "int4_budget_bytes": int(int4_budget_bytes),
            "bf16_page_size_bytes": int(bf16_page_size),
            "int4_page_size_bytes": int(int4_page_size),
            "bf16_num_blocks": int(bf16_blocks),
            "int4_num_blocks": int(int4_blocks),
        }
    )
    return int(int4_blocks), updated_budget
...
def get_kv_cache_config_from_groups(
    ...
    reflex_int4_budget: dict[str, int | float] | None = None
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        num_blocks, reflex_int4_num_blocks, reflex_int4_budget = (
            get_reflex_int4_budgeted_block_counts(
                vllm_config,
        ...
        num_blocks = may_override_num_blocks(vllm_config, num_blocks)
        if (
            reflex_int4_budget is not None
            and vllm_config.cache_config.num_gpu_blocks_override is not None
        ):
            reflex_int4_num_blocks, reflex_int4_budget = (
                rebudget_reflex_int4_for_bf16_blocks(
                    kv_cache_groups[0].kv_cache_spec,
                    available_memory=available_memory,
                    bf16_blocks=num_blocks,
                    budget_info=reflex_int4_budget,
                )
            )
    ...
    return KVCacheConfig(
        ...
        if reflex_int4_budget is not None
        else None,
        reflex_int4_budget=reflex_int4_budget,
    )
