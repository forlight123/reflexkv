...
if TYPE_CHECKING:
    ...
    ReflexDemotion = object
    ReflexRecovery = object
...
class SchedulerOutput:
    ...
    # Dynamic ReFlexKV BF16->INT4 demotions that must be executed by the
    # worker before any newly allocated BF16 blocks are zeroed or reused.
    reflex_int4_demotions: list[ReflexDemotion] | None = None

    # Dynamic ReFlexKV INT4->BF16 recoveries that must be executed by the
    # worker before the recovered BF16 block table entries are used.
    reflex_int4_recoveries: list[ReflexRecovery] | None = None
