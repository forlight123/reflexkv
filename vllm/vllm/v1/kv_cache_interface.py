...
class AttentionSpec(KVCacheSpec):
    ...
    def int4_packed_head_size_bytes(head_size: int) -> int:
        ...
        # Two signed 4-bit values are packed into one byte. The extra byte is a
        # power-of-two scale exponent for this token/head vector.
    ...
    def real_page_size_bytes(self) -> int:
        if self.cache_dtype_str == "int4":
            packed_head_size = self.int4_packed_head_size_bytes(self.head_size)
            return 2 * self.block_size * self.num_kv_heads * packed_head_size
        return (
            2
            * self.block_size
            * self.num_kv_heads
...
class KVCacheConfig:
    ...
    """The number of INT4 KV sidecar blocks for ReFlexKV."""
    reflex_int4_budget: dict[str, int | float] | None = None
    """Budget accounting metadata for ReFlexKV mixed BF16/INT4 storage."""
