...
class CacheConfig:
    ...
    def _apply_block_size_default(self) -> "CacheConfig":
        ...
        if self.cache_dtype == "reflex_int4" and self.enable_prefix_caching:
            logger.warning(
                "Disabling prefix caching for experimental reflex_int4 KV "
                "cache. The first dynamic INT4 prototype does not demote "
    ...
    def _validate_cache_dtype(cls, cache_dtype: CacheDType) -> CacheDType:
        ...
        if cache_dtype == "reflex_int4":
            logger.warning(
                "Using experimental reflex_int4 KV cache. New KV pages are "
                "stored in the model dtype, while sealed pages may be migrated "
                "to a separate packed INT4 pool under serving pressure."
            )
        return cache_dtype
