...
class P2pNcclEngine:
    ...
    def recv_tensor(
        ...
        if self.send_type == "PUT" or self.send_type == "PUT_ASYNC":
            ...
            if _semantiq_p2p_debug_enabled():
                logger.info(
                    "SemantiQ P2P recv wait: tensor_id=%s remote=%s rank=%d",
                    tensor_id,
                    remote_address,
                    self.rank,
                )
            ...
            if _semantiq_p2p_debug_enabled():
                logger.info(
                    "SemantiQ P2P recv_store hit: tensor_id=%s tensor_type=%s "
                    "rank=%d wait_ms=%.3f",
                    tensor_id,
    ...
    def listen_for_requests(self):
        while True:
            ...
            if data["cmd"] == "NEW":
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P listener PUT begin: tensor_id=%s "
                        "remote=%s shape=%s dtype=%s rank=%d",
                        tensor_id,
                        remote_address.decode(),
                        data.get("shape"),
                        data.get("dtype"),
                ...
                try:
                    ...
                    if _semantiq_p2p_debug_enabled():
                        logger.info(
                            "SemantiQ P2P listener PUT received: tensor_id=%s "
                            "shape=%s rank=%d",
                            tensor_id,
                            tuple(tensor.shape),
                            self.rank,
    ...
    def send_async(self):
        while True:
            ...
            try:
                ...
                logger.exception(
                    "SemantiQ P2P async send failed: tensor_id=%s remote=%s",
                    item.tensor_id,
                    item.remote_address,
                )
                raise
    ...
    def send_sync(self, item: SendQueueItem) -> bool:
        ...
        if _semantiq_p2p_debug_enabled():
            logger.info(
                "SemantiQ P2P send PUT begin: tensor_id=%s remote=%s "
                "shape=%s dtype=%s rank=%d",
                item.tensor_id,
                item.remote_address,
                tuple(tensor.shape),
                str(tensor.dtype),
        ...
        if _semantiq_p2p_debug_enabled():
            logger.info(
                "SemantiQ P2P send NCCL begin: tensor_id=%s remote=%s rank=%d",
                item.tensor_id,
                item.remote_address,
                self.rank,
            )
        ...
        if _semantiq_p2p_debug_enabled():
            logger.info(
                "SemantiQ P2P send NCCL done: tensor_id=%s remote=%s rank=%d",
                item.tensor_id,
                item.remote_address,
                self.rank,
            )
    ...
    def get_finished(
        ...
        for request_id in finished_req_ids:
            tensor_ids = set(self.recv_request_id_to_tensor_ids.get(request_id, set()))
            tensor_ids.update(
