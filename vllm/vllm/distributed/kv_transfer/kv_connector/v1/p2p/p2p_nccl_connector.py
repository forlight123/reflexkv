...
def _semantiq_p2p_debug_enabled() -> bool:
    return os.environ.get("SEMANTIQ_P2P_DEBUG", "0") == "1"


def _adapt_kv_cache_layout_for_injection(
    layer: torch.Tensor,
    kv_cache: torch.Tensor,
) -> torch.Tensor:
    """Adapt KV cache layout across attention backends before P2P injection.

    vLLM's P2P connector can connect a prefill instance and a decode instance
    using different attention backends. In the ReFlexKV 1P1D path, prefill
    keeps the default FlashAttention layout ``[2, num_blocks, ...]`` while the
    decode instance uses the Triton/FlashInfer-style layout
    ``[num_blocks, 2, ...]`` for its BF16 primary KV cache.
    """
    if layer.ndim == kv_cache.ndim and layer.ndim >= 2:
        layer_is_block_first = layer.shape[1] == 2
        cache_is_kv_first = kv_cache.shape[0] == 2
        layer_is_kv_first = layer.shape[0] == 2
        cache_is_block_first = kv_cache.shape[1] == 2

        if layer_is_block_first and cache_is_kv_first:
            return kv_cache.transpose(0, 1)
        if layer_is_kv_first and cache_is_block_first:
            return kv_cache.transpose(0, 1)
    return kv_cache


def _get_p2p_kv_chunk_blocks() -> int:
    raw_value = os.environ.get("SEMANTIQ_P2P_KV_CHUNK_BLOCKS", "0")
    try:
        return max(0, int(raw_value))
    except ValueError:
        logger.warning(
            "Invalid SEMANTIQ_P2P_KV_CHUNK_BLOCKS=%r, disabling chunked P2P KV",
            raw_value,
        )
        return 0


def _iter_kv_block_chunks(
    ...
    if num_blocks == 0:
        return
    if chunk_blocks <= 0 or chunk_blocks >= num_blocks:
        yield 0, num_blocks, block_ids
        return
    for start in range(0, num_blocks, chunk_blocks):
        end = min(start + chunk_blocks, num_blocks)
        yield start, end, block_ids[start:end]


def _p2p_kv_tensor_id(
    request_id: str,
    layer_name: str,
    start: int,
    end: int,
    ...
    base_id = request_id + "#" + layer_name
    if start == 0 and end == total_blocks:
        return base_id
    return f"{base_id}#blocks{start}:{end}"


def _slice_kv_cache_blocks(
    kv_cache: torch.Tensor,
    block_dim: int,
    start: int,
    end: int,
) -> torch.Tensor:
    return kv_cache.narrow(block_dim, start, end - start)
...
class P2pNcclConnector(KVConnectorBase_V1):
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        def inject_kv_into_layer(
            ...
            """
            kv_cache = _adapt_kv_cache_layout_for_injection(layer, kv_cache)
            if (
        ...
        if _semantiq_p2p_debug_enabled():
            logger.info(
                "SemantiQ P2P load meta: requests=%d layers=%d",
                len(metadata.requests),
                len(forward_context.no_compile_layers),
            )

        chunk_blocks = _get_p2p_kv_chunk_blocks()
        ...
        for request in metadata.requests:
            ...
            for layer_name in forward_context.no_compile_layers:
                ...
                for start, end, chunk_block_ids in _iter_kv_block_chunks(
                    request.block_ids, chunk_blocks
                ):
                    tensor_id = _p2p_kv_tensor_id(
                        request.request_id,
                        layer_name,
                        start,
                        end,
                    ...
                    if _semantiq_p2p_debug_enabled():
                        logger.info(
                            "SemantiQ P2P recv begin: request_id=%s layer=%s "
                            "remote=%s blocks=%d:%d/%d tensor_id=%s",
                            request.request_id,
                            layer_name,
                            remote_address,
                            start,
                    ...
                    if _semantiq_p2p_debug_enabled():
                        logger.info(
                            "SemantiQ P2P recv done: request_id=%s layer=%s "
                            "blocks=%d:%d shape=%s",
                            request.request_id,
                            layer_name,
                            start,
                    ...
                    if kv_cache is None:
                        raise RuntimeError(
                            "P2P KV transfer failed before injection: "
                            f"request_id={request.request_id} layer={layer_name} "
                            f"blocks={start}:{end}"
                        )

                    inject_kv_into_layer(
                        layer, kv_cache, chunk_block_ids, request.request_id
                    )
                    del kv_cache
        ...
        **kwargs: Any,
        ...
        def extract_kv_from_layer(
            ...
            if (
                ...
                return layer[block_ids, ...], 0

            if layer.shape[0] == 2:  # FlashAttention
                return layer[:, block_ids, ...], 1

            return None, None

        connector_metadata = self._get_connector_metadata()
        assert isinstance(connector_metadata, P2pNcclConnectorMetadata)
        if _semantiq_p2p_debug_enabled() and connector_metadata.requests:
            logger.info(
                "SemantiQ P2P save layer: layer=%s requests=%d",
                layer_name,
        ...
        for request in connector_metadata.requests:
            ...
            for start, end, chunk_block_ids in _iter_kv_block_chunks(
                ...
                kv_cache, block_dim = extract_kv_from_layer(kv_layer, chunk_block_ids)
                if kv_cache is None or block_dim is None:
                    continue
                if start != 0 or end != total_blocks:
                    kv_cache = _slice_kv_cache_blocks(
                        kv_cache,
                        block_dim,
                        0,
                        len(chunk_block_ids),
                ...
                tensor_id = _p2p_kv_tensor_id(
                    request_id, layer_name, start, end, total_blocks
                )
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P send enqueue: request_id=%s layer=%s "
                        "remote=%s blocks=%d:%d/%d shape=%s tensor_id=%s",
                        request_id,
                        layer_name,
                        remote_address,
                        start,
    ...
    def build_connector_meta(
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            if self.is_producer:
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P producer new_req: request_id=%s "
                        "scheduled=%d computed=%d prompt=%d blocks=%d",
                        new_req.req_id,
                        num_scheduled_tokens,
                        new_req.num_computed_tokens,
                        len(new_req.prompt_token_ids or []),
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P producer meta add new_req: "
            ...
            if new_req.req_id in self._requests_need_load:
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P consumer meta add new_req: "
                        "request_id=%s blocks=%d",
                        new_req.req_id,
                        len(new_req.block_ids[0]),
                    )
        ...
        for i, req_id in enumerate(cached_reqs.req_ids):
            ...
            if self.is_producer:
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P producer meta add cached_req: "
                        "request_id=%s blocks=%d",
                        req_id,
                        len(block_ids),
                    )
            ...
            if req_id in self._requests_need_load:
                ...
                if _semantiq_p2p_debug_enabled():
                    logger.info(
                        "SemantiQ P2P consumer meta add resumed_req: "
                        "request_id=%s blocks=%d",
                        req_id,
                        len(block_ids),
                    )
        ...
        if _semantiq_p2p_debug_enabled() and meta.requests:
            logger.info(
                "SemantiQ P2P build meta done: is_producer=%s requests=%d",
                self.is_producer,
                len(meta.requests),
            )
        return meta
