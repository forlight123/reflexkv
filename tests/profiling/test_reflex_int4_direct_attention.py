...
class ReflexInt4DirectAttentionTest(unittest.TestCase):
    ...
    def test_direct_attention_reads_bf16_tile_when_reflex_cache_is_bound(self):
    ...
    def test_direct_attention_reads_all_int4_pages(self):
        device = select_reflex_cuda_test_device()
        dtype = torch.bfloat16
        block_size = 2
        head_size = 16
        ...
        int4_key = (
            ...
            .view(2 * block_size, num_kv_heads, head_size)
            .sub(15.0)
            .to(dtype)
        )
        int4_value = (int4_key.float() * 0.25 + 2.0).to(dtype)
        int4_quantize_and_cache(
            int4_key,
            int4_value,
            int4_cache,
    ...
    def test_3d_decode_reads_all_int4_pages(self):
        device = select_reflex_cuda_test_device()
        dtype = torch.bfloat16
        block_size = 2
        head_size = 16
